...
function back( bReedit:boolean ){
	if ( !bReedit && 0 < gb.curIndex )
		gb.curIndex--
...
function enter( vh:any ):void{
	...
	if ( v.length >= gb.maxLetter )
	{
		if ( word == v ){
			const cb = ()=>{
				let msg = 'Genius!', svenMsg = 'MY MASTER!!!', t = 9000;
				switch (gb.curLine){
					case 1: msg = 'Magnificent!'; svenMsg = 'CONGRATS!!!'; t=7200; break
					case 2: msg = 'Impressive!'; svenMsg = 'WOW!!!'; t=7200; break
					case 3: msg = 'Splendid!'; svenMsg = 'YAY!!'; t=7200; break
					case 4: msg = 'Great!'; break
					case 5: msg = 'Good!'; break
				}
				if ( gb.curLine <= 3 )
					threeProc( vh, svenMsg );
				else
					t = 3000

				sq('#idFrame section').eq(gb.curLine).find('div').each(function(this:any, idx:number){
					sq(this).css('animation-delay', `${idx*100}ms`).addClass('cSolved')
				})
...
function vSQProc( vh:any ):void{
		...
		if ( isNaN(line) || (line != gb.curLine || !gb.bGaming) )
		{
			if ( !isNaN(line) && (line < gb.curLine || !gb.bGaming) )
			{
				...
				if ( w.length == gb.maxLetter )
					window.open( 'https://www.google.com/search?tbm=isch&q=' + encodeURIComponent(w + ' meaning'), '_blank')?.focus();
			}
			return
		}
	...
	sq('body').on('keydown', function( e:any ){
		...
		const bReedit = sq('#idFrame section').find('.reedit').length > 0
		sq('*').removeClass('reedit')
		g_keyStates[k] = true
		switch ( k ){
			case 'enter': case '↵':
		...
		if ( k == 'enter' )
		{
			if ( sq('#idGenText').length && document.activeElement == sq('#idGenText').get(0) )
			{
				if ( sq('#idGenText').val().trim()?.length == 5 )
				{
					window.open(sq('#idOAnswerLink').attr('href'), '_blank')?.focus()
					return false
				}
			}
		}
	...
	function resizeY(){
		...
		const by = sq('body').height(), wy = sq(window).height()
		if ( by > wy )
		{
			const r = wy / by - 0.02
			sq('#app').css({
